...
var getFlatDependencies = require("../modules/get-flat-dependencies");

module.exports = function settleExclusiveBundleModules(bundles) {
  ...
  _.each(bundles, function (bundleA) {
    _.each(bundles, function (bundleB) {
      var intersection;
      if (bundleA !== bundleB) {
        intersection = _.intersection(bundleA.moduleHashes, bundleB.moduleHashes);
        if (intersection) {
          ...
          bundles.push({
            ...
            dest: defaults.implicitBundleDest
          });
        }
      }
    });
