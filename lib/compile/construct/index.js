...
import { programTmpl, bodyTmpl } from "../../ast/template";
...
const registerUrlsTmplPath = path.join(__dirname, "templates/register-urls.jst");
const registerUrlsTmplStr = fs.readFileSync(registerUrlsTmplPath, "utf-8");
const registerUrlsTmpl = bodyTmpl(registerUrlsTmplStr);

const iifeTmplPath = path.join(__dirname, "templates/iife.jst");
const iifeTmplStr = fs.readFileSync(iifeTmplPath, "utf-8");
const iifeTmpl = programTmpl(iifeTmplStr);
...
export function constructModuleSet (cjsModules, globalName) {
  ...
  return toArray(cjsModules)
    ...
    .then(modulesAst =>
      b.objectExpression(modulesAst))
    .then(modulesHashAst =>
      moduleSetTmpl({ globalName: b.literal(globalName), modulesHashAst }).body);
}
