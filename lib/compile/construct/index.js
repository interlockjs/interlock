...
import { programTmpl } from "../../ast/template";
...
export function constructBundle (opts) {
  ...
  return Promise.all([runtimeP, urlsP, moduleSetP, initialRequireP])
    .then(([runtime, urls, moduleSet, initialRequire]) =>
      iifeTmpl()
        .replaceBody("BODY", [].concat(runtime, urls, moduleSet, initialRequire).filter(x => x))
        .exec()
    );
}
