...
function compileModules (seedModules) {
  ...
  function* genModules (module) {
    ...
    const deepDependencies = Array.prototype.concat.apply(
      module.dependencies,
      module.dependencies.map(dep => dep.deepDependencies)
    );
    module.deepDependencies = _.chain(deepDependencies).indexBy("hash").values().value();
