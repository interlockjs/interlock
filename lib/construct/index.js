...
construct = module.exports = {
  ...
  commonModule: function constructCommonModule(moduleBody, deps) {
    var depsLiterals = _.map(deps, function (dependencyHash) {
  ...
  runtime: function constructRuntime(globalName) {
    return runtimeTmpl({ globalName: b.literal(globalName) }).body;
  },
  ...
  bundle: function constructBundle(opts) {
    ...
    if (opts.modules) {
      commonModules = opts.modules.map(function (moduleObj) {
        return construct.commonModule(moduleObj.ast.body[0], moduleObj.deps);
      });
